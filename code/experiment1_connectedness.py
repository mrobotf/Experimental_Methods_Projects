...
def create_trial_list(reference_patterns, test_patterns, block_num):
    ...
    for idx in indices:
        ...
        # Randomize which side gets test pattern
    ...
    # Second half: same order, reversed positions
    for idx in indices:
        ref_pattern = reference_patterns[idx]
        test_pattern = test_patterns[idx]
        
        # Reverse the position from first half
...
def create_practice_trials(reference_patterns):
    ...
    for i in range(NUM_PRACTICE_TRIALS):
        # Create simple test pattern with 9 dots
...
def run_experiment():
    ...
    # Generate all patterns (this takes time!)
...
Press SPACE to begin practice."""
    ...
    for block_num in range(1, NUM_BLOCKS + 1):
        ...
        for i, trial in enumerate(trials):
            ...
            # Optional: progress indicator every 50 trials
            if (i + 1) % 50 == 0:
                progress = stimuli.TextScreen(
