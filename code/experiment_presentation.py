...
FIXED VERSION: Improved line crossing detection to prevent overlapping lines
"""
...
class DotPattern:
            ...
            for attempt in range(max_attempts):
                ...
                for idx2 in available_dots[1:]:
                    ...
                    if MIN_LINE_LENGTH <= distance <= MAX_LINE_LENGTH:
                        ...
                        # IMPROVED: Check both crossing AND proximity to other lines
    ...
    def _generate_free_lines(self, num_lines):
        ...
        for _ in range(num_lines):
            ...
            for attempt in range(max_attempts):
                ...
                # IMPROVED: Check crossing, proximity to other lines, and proximity to dots
    ...
    def _lines_intersect(self, line1, line2):
        ...
        Uses parametric line equation method
        """
        (x1, y1), (x2, y2) = line1
        (x3, y3), (x4, y4) = line2
        ...
        # Use strict inequality to avoid endpoint touching
        return 0.01 < t < 0.99 and 0.01 < u < 0.99

    def _line_too_close_to_other_lines(self, new_line, existing_lines):
        """
        NEW METHOD: Check if line is too close to any existing line
    ...
    def _point_to_segment_distance(self, point, seg_start, seg_end):
        ...
        # Parameter t represents the projection point on the line
        # t=0 is seg_start, t=1 is seg_end
...
def create_pattern_stimulus(pattern, offset_x):
    """Create Canvas with pattern at specified horizontal offset"""
...
def generate_pattern_set(num_dots_test):
    ...
    This ensures that differences are only due to connectedness, not dot placement
    """
    # Create base pattern with 0 connections
    base_pattern = DotPattern(num_dots_test, connectedness=0)
    base_pattern.generate()

    # Copy the dot configuration to reuse for other connectedness levels
    base_dots = base_pattern.copy_dots()

    # Create patterns with same dots but different connectedness
